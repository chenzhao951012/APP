        ...
        <view class="cmd-progress-inner" :style="setCircleStyle">
          ...
          <block v-if="showInfo">
            <text class="cmd-progress-text" :title="setFormat">
              <block v-if="status != 'success' && status != 'exception' && setProgress < 1000">{{setFormat}}0</block>
              <!-- #ifdef H5 -->
              ...
              <!-- #endif -->
              <!-- #ifndef H5 -->
              <text v-if="status == 'exception' || status == 'success' || setProgress == 1000" :style="setCircleIcon"></text>
              <!-- #endif -->
            </text>
          </block>
          <!-- 状态文本 end -->
