<template>
	<view class="content">
		<view class="massageBox">
			...
			<view class="massageList">
	<div class="container">
    <!-- 头部 -->
    <!-- 内容 -->
    <div class="infoAll" v-for="(item,index) in commitInfo" :key="index+10">
      <ul v-if="item.top">
        <li @touchstart="touchStart($event)" @touchend="touchEnd($event,index+10)" :data-type="item.type" style="background-color:#EDFBFE;">
          <div class="imgInfo" @click="recover(index+10)">
            <img :src="item.img">
          </div>
